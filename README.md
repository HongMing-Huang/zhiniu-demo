...
| 移动端 4 Tab | 行情 / 自选 / AI研究 / 我的 底部导航（对标 KuiklyStock）；「我的」页：用户卡 + 资产统计 + 外观（浅/深/跟随系统）+ **涨跌配色切换**（红涨绿跌↔绿涨红跌，全 App 即时生效，SP 持久化）+ 服务状态 + 关于/免责声明 |
| 全屏搜索（手机） | 首页大搜索栏 → 全屏搜索页：热门榜单化（序号热红）/ 最近查看 / 东财 suggest 全市场 / 结果带真实价与涨跌幅；桌面保留下拉面板 |
...
| 流式 AI 问答 | 通用问答走 `/agent/chat/stream` typed SSE 逐字上屏（打字机）；回复末尾 `【AI观点】` 服务端结构化为 **操作建议 + 风险徽章**；`[KCHART:代码]` 指令渲染**真实日 K 迷你图**；双股对比页 `/agent/compare/stream` 流式归纳（stronger 标记 + 双列依据 + 结论），阶段进度可见 |
| **AI Agent 工具指令** | 对知牛说「把比亚迪加到自选」「宁德时代跌破 300 提醒我」「对比茅台和宁德」「换成深色」「配色换成绿涨红跌」——模型按 ⟦TOOL⟧ 协议输出指令（共 6 条：加自选 / 双股对比 / 价格预警 / 转入研究 / 切外观 / 切涨跌配色），服务端白名单校验 + **名称→代码经东财搜索真实解析（拒绝编造）** + 无指令强重试，客户端执行并回执反馈卡；自选、预警、外观、配色均落 SharedPreferences 持久化，真实行情到达自动标记「已触发」 |
| 会话本地归档 | 会话与聊天消息 SharedPreferences 持久化，刷新 / 冷启动不丢记录（恢复正文、工具反馈卡与结论徽章；外观与涨跌配色偏好同步持久化） |
...
| Light/Dark/System | 语义色 token + 动态调色板代理，180ms 平滑切换，H5/原生一致；涨跌配色经同一代理全局反转 |
