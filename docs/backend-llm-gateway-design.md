...
> 文档版本：v0.1（调研定稿，待实现）
> 日期：2026-08-22
> 状态：**设计提案，需评审** —— 与根目录 `DEVELOPMENT-ISSUES.md` 逐条对齐后进入实现
...
「问股对话」顶部的时间线（基本面→技术→舆情→风控）需要网关在**工具执行阶段**向客户端推送事件。SSE 中除 content 增量外，网关可额外发带 `event: agent_progress` 的自定义事件，客户端据此点亮进度，无需在客户端模拟。
