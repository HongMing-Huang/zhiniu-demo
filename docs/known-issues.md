...
- **状态**：🔶 待修（P1）。已加 `toggleAiPanel()` 诊断日志（`zn-detail: toggle → …`），下一次真机手指点击即可确认是「事件未触发」还是「面板渲染问题」；注意 `adb input tap` 自动化注入在该 Kuikly 按钮上无法触发（真机手指与注入事件管线不同，需真机复验）。同时已确认：顶部行情回退 Mock 与「AI 按钮」无关——行情页 15s 自动刷新正常到达网关（真实数据），详情页 quotes/kline 请求亦正常到达并成功（`quotes fetch → 337.69/新浪财经`），此前误判为点击导致。
