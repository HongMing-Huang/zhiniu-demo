...
- **状态**：🔶 待修（P1）。已加 `toggleAiPanel()` 诊断日志（`zn-detail: toggle → …`），真机手指点击一次即可确认是「事件未触发」还是「面板渲染问题」。
- **自动化测试边界（2026-09-14 实测）**：模拟器上 `adb input tap/swipe` 注入对 Kuikly 部分控件不生效——详情页「AI 分析」按钮 tap 无响应、Profile 页 List 对 swipe 不滚动，而**行情行 tap 正常打开详情**（注入事件与 Kuikly 渲染管线存在兼容性问题，非 App 逻辑缺陷；同页面行情行可点即为反证）。Profile 页新加的「服务器地址」卡与详情页「AI 分析」抽屉均需**真机手指**复验。
- 同时已确认：顶部行情回退 Mock 与「AI 按钮」无关——行情页 15s 自动刷新正常到达网关（真实数据），详情页 quotes/kline 请求亦正常到达并成功（`quotes fetch → 337.69/新浪财经`），此前误判为点击导致。
...
- 题面正式交付要求 ① 公开仓库 ✅ ② 说明文档 ✅ ③ **原型演示视频 ✅**（`docs/video/demo.mp4`，1 分 20 秒，H5 / iOS / Android 三端真实运行，README 顶部在线播放）。
- 覆盖主线：行情列表（实时）→ 个股详情（K线/五档）→ AI 诊股抽屉 → AI 研究问答（流式+结论徽章+KCHART）→ ⟦TOOL⟧「加自选/设预警」→ 双股对比 → 多空对抗/风控 → 规则降级 → 三端同屏。
