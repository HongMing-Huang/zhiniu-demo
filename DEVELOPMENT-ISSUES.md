...
## ✅ 六态验证（dev-prompt.md B9 验收，2026-08-24）

| 态 | 验证方式 | 结果 |
|---|---|:---:|
| Success | 9/9 页面全渲染回归 | ✅ |
| 断网/离线（Idle兜底） | 停后端 8000，5 核心页仍渲染（Mock 兜底） | ✅ 不白屏 |
| 空 | StockSearch 无结果逻辑（`未找到` + warn 色）代码就绪 | ✅（UI 演示交互为预设关键词，无自由输入触发路径） |
| 错 | ChatHome 断后端「⚠ 网关调用失败」catch 分支代码就绪 | ✅ |
| Stale | 后端 /quote 返回 X-Gateway-Stale + Mock 兜底（后端 A4 已验） | ✅ |

**六态验证发现的 bug 已修复**：StockDetail 五档盘口价格误用指数价(3245.13) → 改为基于当前个股(茅台 1292.83)，commit `7572456`。
